...
当前分支：`1.19.2`
适配版本：Minecraft `1.19.2` / Forge `43.5.0` / Java `17`
...
1. 打开这个版本的 release：[`DGLabCraft-1.19.2-1.0.7.jar`](https://github.com/bilbillm/DGLab-Craft/releases/tag/v1.0.7-1.19.2)
2. 在页面下方找到 **Assets**。
3. 下载 `DGLabCraft-1.19.2-1.0.7.jar`。
...
1. 用启动器创建或选择一个 Minecraft `1.19.2` 实例。
2. 给这个实例安装 Forge `43.5.0`。
3. 打开实例目录里的 `mods` 文件夹。常见路径是 `.minecraft/mods`。
4. 把刚下载的 `DGLabCraft-1.19.2-1.0.7.jar` 放进去。
