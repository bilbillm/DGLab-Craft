...
当前分支：`1.12.2-Forge`
适配版本：Minecraft `1.12.2` / Forge `14.23.5.x` / Java `8`
...
| --- | --- | --- | --- |
| 1.12.2 | Forge 14.23.5.x | `1.12.2-Forge` | [v1.1.0](https://github.com/bilbillm/DGLab-Craft/releases/tag/v1.1.0-1.12.2) |
...
1. 打开这个版本的 release：[`DGLabCraft-1.12.2-1.1.0.jar`](https://github.com/bilbillm/DGLab-Craft/releases/tag/v1.1.0-1.12.2)
2. 在页面下方找到 **Assets**。
3. 下载 `DGLabCraft-1.12.2-1.1.0.jar`。
...
1. 用启动器创建或选择一个 Minecraft `1.12.2` 实例。
2. 给这个实例安装 Forge `14.23.5.x`。
3. 打开实例目录里的 `mods` 文件夹。常见路径是 `.minecraft/mods`。
4. 把刚下载的 `DGLabCraft-1.12.2-1.1.0.jar` 放进去。
...
```bash
git clone -b 1.12.2-Forge https://github.com/bilbillm/DGLab-Craft.git
cd DGLab-Craft
./gradlew build
```
...
```bash
pwsh scripts/release-check.ps1 -Tag v1.1.0-1.12.2 -SkipGitHubReleaseCheck
pwsh scripts/release-postcheck.ps1 -Tag v1.1.0-1.12.2
```
