...
public class EnvironmentHandler {
    ...
    private void handleClientEnvironment(Player player) {
        ...
        if (!isNetherDimension) {
            ...
            if (hasSnowNearby) {
                ...
                // 高频细碎的麻木感
                WebSocketManager.getInstance().sendStimulus("A", "pulse", intensity, 100);
            } else {
                wasInCold = false;
            }
        }
    ...
    private void checkPlayerFootBlock(Player player) {
        ...
        if (feetBlock == Blocks.SLIME_BLOCK) {
            double intensity = 0.1;
            WebSocketManager.getInstance().sendStimulus("A", "square", intensity, 150);
        }

        // 蜘蛛网 - 轻微束缚感
        if (feetBlock == Blocks.COBWEB) {
            double intensity = 0.12;
            WebSocketManager.getInstance().sendStimulus("A", "pulse", intensity, 150);
        }
        ...
        if (feetBlock == Blocks.ENCHANTING_TABLE || feetBlock == Blocks.BOOKSHELF) {
            ...
            WebSocketManager.getInstance().sendStimulus("B", "pulse", intensity, 250);
        }
        ...
        if (feetBlock == Blocks.MOSSY_COBBLESTONE || feetBlock == Blocks.STONE) {
            ...
            WebSocketManager.getInstance().sendStimulus("B", "sine", intensity, 400);
        }
