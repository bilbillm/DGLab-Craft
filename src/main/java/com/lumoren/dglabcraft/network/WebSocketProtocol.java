...
final class WebSocketProtocol {
    ...
    static int leaseTicks(WebSocketServerManager.EffectSource source, String detail) {
        ...
        if (source == WebSocketServerManager.EffectSource.ENVIRONMENT) {
            if ("portal".equals(normalizedDetail) || "powder_snow".equals(normalizedDetail)) {
                return 40;
            }
            if ("nether".equals(normalizedDetail) || "end".equals(normalizedDetail)) {
                return 55;
            }
            return 45;
        }
        if (source == WebSocketServerManager.EffectSource.DAMAGE) {
            if ("onfire".equals(normalizedDetail) || "infire".equals(normalizedDetail) || "lava".equals(normalizedDetail)
                || "hotfloor".equals(normalizedDetail) || "drown".equals(normalizedDetail) || "freeze".equals(normalizedDetail)) {
                return 30;
            }
            return 12;
        }
