...
public class DGLabCraftHUD {
    ...
    static void renderHudPanel(PoseStack poseStack, Font font, WebSocketServerManager server,
        ...
        int x = rect.x() + 8;
        int y = rect.y() + 6;
        font.draw(poseStack, translatable("overlay.dglabcraft.hud_title"), x, y, YELLOW);

        if (!server.isConnected()) {
            font.draw(poseStack, translatable("overlay.dglabcraft.disconnected"), x, y + 14, RED);
            String keyName = ClientModEvents.OPEN_SETTINGS_KEY.get().getKey().getDisplayName().getString();
            font.draw(poseStack, translatable("overlay.dglabcraft.open_settings_hint", keyName), x, y + 28, MUTED);
            return;
        }

        font.draw(poseStack, translatable("overlay.dglabcraft.connected"), x, y + 14, GREEN);
        font.draw(poseStack, translatable("overlay.dglabcraft.channel_state",
